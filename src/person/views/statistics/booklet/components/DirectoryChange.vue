    ...
    <a-form :form="form" class="directory-form">
      <a-row>
        <a-form-item label="上一级目录">
          <a-tree-select
            ...
            v-decorator="[
              'parent',
              { initialValue: this.editData.pid || undefined, rules: [{ required: false, message: '请选择上一级目录' }] },
            ]"
          />
        </a-form-item>
      </a-row>
      <a-row>
        <a-form-item label="目录名称" :wrapper-col="{ span: 25}">
          <a-input
            v-decorator="['name', { initialValue: this.record.name, rules: [{ required: true, whitespace:true, message: '请输入目录名称'}] }]"
          />
        </a-form-item>
      </a-row>
      <a-row>
        <a-form-item label="排序">
          <a-input-group compact>
            <a-select
              ...
              allowClear
              style="width: 87%"
              v-decorator="[ 'toid', { initialValue: undefined, rules: [{ required: false, message: '请添加排序'}] }]"
              >
              <a-select-option
            ...
            <a-select style="width: 13%" v-decorator="[ 'before',{ initialValue: '1' }]">
      ...
      <a-row>
        <a-form-item label="绑定模板">
          <a-select
            ...
            v-decorator="[
              'tempid',
              { initialValue: this.editData.tempname, rules: [{ required: true, message: '请选择模板'}] },
            ]"
          >
            <!-- 输入加载 -->
  ...
  methods: {
    ...
    getsortList(query){
     ...
     catalogspost(query)
      .then(({result}) => {
        ...
        this.sortList = arr;
        this.disableSortList(this.sortList);
      })
      .catch(err => {
        showError(err);
      })
    ...
    async loadCatalog() {
      ...
      if(this.record.id) {// 存在id 编辑 否则新增
        ...
        catalogDetail(this.record.id).then(({result}) => {
