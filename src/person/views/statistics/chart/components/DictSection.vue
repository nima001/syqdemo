<template>
  <div class="dict-section-panel">
    ...
    <input-modal v-model="optItem.show" 
      title="分组名称"
      :value="optItem.name"
      placeholder="分组名称"
      :callback="optItem.callback"
    />
  </div>
</template>
<script>
import { Icon, Button, Checkbox, Input, Dropdown, Menu } from 'ant-design-vue'
    ...
    onItemAdd(){
      this.optItem = {
        ...
        callback: (name) => {
          ...
          this.section.items.push({name: name, values: []});
        }
      };
    },
    onAddToGroup(e){
