<template>
  <a-collapse v-model="activeKey" :bordered="false" style="background-color: unset">
    <a-collapse-panel
      ...
      header="y轴"
      :disabled="chartData.settings.yAxis.visible == false"
    >
      ...
      <a-collapse v-model="activeKey" :bordered="false" style="background-color: unset">
        <a-collapse-panel
          ...
          key="10"
          header="标题"
          :disabled="chartData.settings.yAxis.title.visible==false"
        >
          <template v-slot:extra>
        ...
        <div class="percent" v-if="chartData.settings.chartType=='stack-bar-chart'">
          <span>显示占比</span>
          <a-switch @click="(check, e) => { e.stopPropagation(); }" v-model="chartData.settings.yAxis.percent"/>
        </div>
        <div class="grid">
          <span>网格线</span>
