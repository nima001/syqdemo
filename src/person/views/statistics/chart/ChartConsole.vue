  ...
  <div class="report-chart-panel">
    ...
    <div class="body">
      ...
      <div class="right">
        ...
        <div class="body">
          <div :class="['left', {'left-view': chartType!='table'}]">
          ...
          <div class="right" v-if="chartCmpts && chartCmpts.length">
            <a-tabs v-model="chartType" tabPosition="left" :tabBarGutter="0">
            ...
            />
             -->
            <SettingTree v-model="value" :properties="propSetting" @change="onSettingsChange" />
          </div>
        </div>
      </div>
    </div>
...
import { merge, cloneDeep, assign, keys, pick, intersection, has, set, get } from "lodash";
...
const chartStruct = {
  ...
  settings: {
    ...
    //     value: undefined,
    //     color: undefined,
    //     fontWeight: false, //false-不加粗 true-加粗
    //   },
    //   sub: {
    //     visible: true,//false时不显示，不设置就默认显示
    //     value: undefined,
    ...
    // },
    // xAxis: {
    //   visible: true,//是否显示x轴 true-显示 false 不显示
    //   grid: false,//是否显示网格线
    //   title: {
    //     visible: true,//是否显示标题 true-显示 false 不显示
    //     content: undefined,//标题内容
    //   },
    // },
    // yAxis: {
    //   visible: true,//是否显示y轴配置 true-显示 false 不显示
    //   grid: true,//是否启用网格线
    //   percent: false,
    //   title: {
    //     visible: true,//是否显示标题 true-显示 false 不显示
    ...
    // },
    // gemo: {
    //   color: [ "#4c84ff", "#72c2ff", "#82e1fc", "#1dbb8a", "#8fe94e", "#fbc800", "#ed9421", "#f25f93", "#ef8bcf", "#c562f1" ],//颜色(数组)
    //   label: {
    //     // toFixed: undefined,
    //     visible: true,
    //     color: undefined,
...
export default {
  ...
  computed: {
    field() {
      return {array: this.chartData.fields};
    },
    comptSet() {
      if(this.chartCmpts) {
        let cmptKyes = keys(this.properties);
        let comptsType = this.chartCmpts.filter((item)=>item.type==this.chartType);
        let keyXor = intersection(comptsType[0].compts, cmptKyes);
        let newcmpt = pick(this.properties, keyXor);
        return newcmpt
      }
    },
    ns() {
    ...
    chartType: {
      ...
      get(){
				...
				if(type && this.chartCmpts && this.chartCmpts.find(item => item.type == type)){
					return type;
				}
      }
		},
  ...
  watch: {
    field(val) {
      set(this.fields,'value',val);
    },
    canvas: {
      deep: true,
      handler(val) {
        set(this.value, 'canvas', val);
      }
    },
    comptSet: {
      deep: true,
      ...
      handler(val) {
        if(val) {
          this.propSetting = cloneDeep(val);
        }
      }
    },
    chart(chart) {
    ...
    range(val) {
      if(has(this.value, 'sort')) {
        let that = this;
        this.$set(this.value.sort, 'range', that.range);
      }
    },
    chartType(val) {
      this.$nextTick(()=>{
        if(has(this.value, 'sort')&&this.value.sort!=this.range) {
          let that = this;
          this.$set(this.value.sort, 'range', that.range);
        }
      })
  ...
  methods: {
    onSettingsChange(path, value, node) {
      if(!node) {
        set(this.value, path, value);
      }else if(path.indexOf('visible')>=0) {
        if(get(this.value, path)) {
          node.expanded = true;
        }else{
          node.expanded = false;
        }
    ...
    onDimensionClick(d) {
      ...
      let isExist = withs.filter((item)=>item.key==d.key);
      if(isExist.length) {
        this.$message.error(`${d.showname}已经添加`);
        return;
      }
      withs.push({
        type,
    ...
    onMeasureClick(i) {
      ...
      let isExist = fields.filter((item)=>item.key==i.key);
      if(isExist.length) {
        this.$message.error(`${i.showname}已经添加`);
        return;
      }
    ...
    aggregate() {
      ...
      assign(this.chartData.settings, this.value);
      this.getQueryContext((context) => {
        ...
        aggregateQuery({
          ...
          .then(({ result }) => {
            ...
            //赋值--修改截取数据的最大下标
            this.range = [0,this.queryData.data.rows.length-1];
          })
          .catch((error) => {
            showError(error);
          })
    ...
    readChartComponent() {
      ...
      for (let key in this.slots) {
        let [node] = this.slots[key]();
        let { title, icon, props, properties, compts } = node.componentOptions.Ctor.extendOptions;
        cmpts.push({ type: key, title, icon, props, compts });
  ...
  & > .body {
    ...
    & > .right {
      ...
      & > .body {
        ...
        min-height: 1px;
        & > .left-view {
          /deep/.chart-view > div:first-child{
            background: #e8e8e8;
            & h2,h3 {
              background: #fff;
            }
        ...
        .setting-tree {
          overflow-y: scroll;
          flex: 1;
          /deep/.setting-node > .header .header-content > .extra.ant-radio-group {
            width: 100%;
            display: flex;
