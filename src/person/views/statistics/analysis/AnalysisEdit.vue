  ...
  <accordion-layout>
    ...
    <div slot="nav" class="nav-panel">
      ...
      <div class="body">
        <a-spin v-if="loading" />
        <content-select
          v-else-if="content && content.length"
      ...
      <div class="footer">
        <a-button type="primary" style="width: 100%" @click="doReport"
          >确定</a-button
        >
      </div>
    </div>
      ...
      <div class="header">
        ...
        >
          ...
          <a-menu slot="overlay" @click="doExport($event.key)">
            <a-menu-item key="pdf"
              ><a-icon type="file-pdf" />导出PDF</a-menu-item
            >
            <a-menu-item key="word"
              ><a-icon type="file-word" />导出Word</a-menu-item
            >
          </a-menu>
        </a-dropdown-button>
        <a-button
    ...
    <a-modal v-model="saveOption.show" title="保存选项" @ok="doSave">
      <a-input
        v-model="saveOption.name"
      ...
      <a-radio-group v-model="saveOption.value">
        ...
        <div class="save-option-desc">
          说明：保存分析条件和分析内容，不保存报告文件。可在常用模板中查看。下次可直接使用模板生成最新的报告。
        </div>
        <a-radio :value="2">仅保存报告</a-radio>
        <div class="save-option-desc">
          说明：保存报告文件（默认PDF格式）。可在已保存中查看，点击可直接预览。
        </div>
...
export default {
  ...
  computed: {
    analyzeType() {
      if (this.$store.getters.dict("report.analyzetype")) {
        return this.$store.getters
          .dict("report.analyzetype")
    ...
    types() {
      ...
      if (this.analyzeType && this.analyzeType.length && this.list.length) {
        this.analyzeType.forEach((item, index) => {
          let filterData = [];
          filterData = this.list.filter(
            (listItem) => listItem.type === item.value
          );
  ...
  watch: {
    filter: {
      ...
      handler(filter) {
        ...
        if (this.filteredList && type && subject && scopetype) {
          //所有条件都选择了返回列表第一条
    ...
    scopetypes(arr) {
      if (
        !this.analyzeTarget ||
        this.analyzeTarget.type != this.filter.type ||
  ...
  methods: {
      ...
      Promise.all([
        ...
        .then(([scope, content]) => {
          ...
          if (this.scope.length > 0) {
            this.scope.forEach((item, index) => {
              let district = item.district;
              if (district) {
                this.scope[index].hint = district;
                this.scope[index].defaultvalue = district;
                this.scope[index].disable = true;
              }
            });
          }
    ...
    async doReport() {
      ...
      this.content.forEach((item) => {
        let selected = indexArr.some(
          (idx) => item.lindex <= idx && idx < item.rindex
        );
        if (selected) {
          if (item.form) {
    ...
    async doSave() {
      ...
      try {
        let name = this.saveOption.name || this.reportName,
          setting,
          report;
        ...
        if (this.saveOption.value == 2 || this.saveOption.value == 3) {
          ...
          report = {
            analyzeid: this.analyzeTarget.id,
            name,
            form: JSON.stringify(this.formConfig),
            data: JSON.stringify(data),
          };
        }
        let { result } = await saveReport({ setting, report });
        if (setting && result) {
          //新增配置模板时，更新配置信息，再次点击保存，不重复新增
          setting.id = result;
