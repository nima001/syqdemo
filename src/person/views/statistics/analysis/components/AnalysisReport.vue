<template>
  <div class="analysis-report" ref="contaner">
    <!-- <div class="form-bg"> -->
    <div class="form-wrap">
...
import watermark from '@framework/utils/watermark'
...
export default {
  ...
  mounted() {
    let title = this.uiConfigs['project.title'];
    if(title){
      let url = this.uiConfigs['project.logo'];
      watermark(url, title).then((img) => {
        this.$refs.contaner.style.backgroundImage = `url('${img}')`
      });
    }
  },
  watch: {
  ...
  methods: {
    ...
    getDocTree(form) {
      //获取文档结构树，并将文档标题按规则添加序号
      let queue = [...form],
        path = [{ level: 0, index: 0, children: [] }];
      while (queue.length > 0) {
        let item = queue.shift();
        if (item.type == "titleBar") {
          //给标题添加序号
          ...
          parent.index++;
          let ordinal = ""; //FIXME sunwen 这里给标题添加序号的方案，不支持标题组件配置隐藏条件，后续调整添加序号由表单内部实现
          path.forEach((item) => {
    ...
    convertSheet(list, data, error = []) {
      ...
      list.forEach((item) => {
        if (item.type == "chart") {
          ...
          if (c !== undefined) {
            //FIXME sunwen 数据为空存在2种情况，1未设置 2组件根据条件不显示；直接去掉该内容页
            if (c.image) {
              sheets.push({
