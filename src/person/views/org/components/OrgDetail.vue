<template>
	<a-tabs :animated="false" class="org-info-panel" type="card" @tabClick="onTabClick">
		...
		<a-tab-pane tab="编制情况" key="3" v-if="isUnit && hasPermit('/person/org/orgStaffDistr/staff')">
			<org-staff-distr :orgid="org._id" category="staff"/>
		</a-tab-pane>
    <a-tab-pane tab="编制情况" key="3-1" v-if="org.orgtype == 3 && hasPermit('/person/org/orgStaffDistr/staff/total')">
      <!-- FXIME 余杭区添加一览表单独授权 -->
      <org-staff-distr :orgid="org._id" category="staff"/>
    </a-tab-pane>
    ...
    <a-tab-pane tab="岗位设置" key="5" v-if="org.unittype == 3 && hasPermit('/person/org/orgStaffDistr/job')"><!-- 事业单位显示岗位 -->
