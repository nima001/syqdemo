          ...
          <template v-if="search.key">
            ...
            <ul v-else class="search-list" @scroll="onScroll">
              <template v-for="(item, index) in search.result" >
                <li :key="item.key" 
                  @click="doCheck(item.field, checkedKeys.checked.indexOf(item.key)<0)"
                >
                  <a-checkbox 
          ...
          <div v-else class="tree-list">
            ...
            >
              <template slot="title" slot-scope="item">
                <custom-icon v-if="item.sort" type="dir" style="margin-right: 4px"/>
                <custom-icon v-else :type="item.icon" class="field-icon"/>
...
export default {
  ...
  methods: {
    ...
    getIcon(field){
      let icon;
      if(field.datatype == 2){//字典
        icon = 'dict';
      }else if(field.datatype == 3){//引用
        icon = 'refer';
      }else if(field.datatype == 4){//输入类型（排除字符串）
