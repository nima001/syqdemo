...
import { foldValueCols } from '@person/components/chart/utils'
...
export default {
  ...
  computed: {
    ...mapState({
      scopedDataList: state => state.equationEditor.scopeData
    }),
    // 作用域数据去重
    filterDatasList() {
      let hash = {};
        ...
        if (item) {
          if(item.aggregate){
            getHeader(item.queryid).then(({result}) => {
              let header = [...result.keyCols, ...foldValueCols(result.valueCols)];
              this.sourceFields = [
