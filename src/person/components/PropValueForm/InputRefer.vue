  ...
  methods: {
    ...
    finishSelect(type, list) {
      ...
      if (type == "ok" && list.length > 0) {
        ...
        if(this.property.multi){
          this.propValue = list.map(item => this.convert(item));
        }else{
          this.propValue = this.convert(list[0])
        }
        // this.validate();
      }
    },
    convert(data){
      const labelKey = this.selectMode == 'user' ? 'username' : 'name';
      const obj = { _id: data._id, [labelKey]: data[labelKey] };
      if(this.selectMode == 'org'){//FIXME sunwen 绍兴项目分析评估模块需要获取组织的hospitaltype字段 待表单异步联动支持去除
        obj.hospitaltype = data.hospitaltype;
      }
      return obj
    }
  }
