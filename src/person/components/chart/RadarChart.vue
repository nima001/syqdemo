<template>
  <div ref="content" class="radar-chart" :style="{width: width?`${width}px`:'100%', height: height?`${height}px`:'400px'}">
...
import { get, has, xor, reduce, map, orderBy, groupBy, keys, assign } from 'lodash';
export default {
  icon: 'chart-radar',
  title: "雷达图",
  name: "BarChart",
  ...
  mixins: [BaseMixin],
  compts: ['canvas', 'title', 'tooltip', 'legend', 'gemo'],
  watch: {
    data: {
      handler(v) {
        return v;
  ...
  methods: {
    ...
    draw(dataTable) {
      ...
      chart
        ...
        .shape('circle')
        .size(4)
        .tooltip('key*value*type',function (key,value,type) {
          return {
            title: key,
            name: type||key,
