<template>
  <div ref="content" class="pie-chart"  :style="{width: width?`${width}px`:'100%', height: height?`${height}px`:'400px'}">
...
import { get, orderBy, cloneDeep, xor, reduce, concat, map, has, assign  } from "lodash";
...
export default {
  ...
  mixins: [BaseMixin],
  compts: ['canvas', 'title', 'tooltip', 'legend', 'gemo', 'sort'],
  data() {
    return {
      dataSource: [],
      currentMoveX: 0,
    ...
    draw(dataTable) {
      ...
      if (this.label&&this.label.visible!=false) {
        geometry.label("key*value", {
          ...
          content: (obj) => `${obj.key}:  ${obj.value.toFixed(that.format.toFixed)}`,
          style: {
