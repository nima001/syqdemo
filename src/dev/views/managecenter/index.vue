...
export default {
  ...
  methods: {
    ...
    // 管理应用
    appmanage(record) {
      this.$router.push({path: "/dev/manage/app",query:{id:record.id,state:record.state,name:record.name}});
    },
    // 展示转让页面
    ...
    showLogoffModal(record) {
      ...
      if(state == 0){
        ...
        this.$notification.warning({message:'提示',description:'转让或起时禁止注销!',duration:1.5})
      }
    },
