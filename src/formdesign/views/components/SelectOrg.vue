  ...
  <div>
    <a-form-item v-if="formLayout != 'horizontal'" :label="name" :required="required" :validateStatus="validateStatus">
      <a-tooltip placement="topLeft" :title="help">
        <!--
          read-only 与 allowClear 属性互斥，所以这里加focus事件
        -->
        <a-input
...
export default {
  ...
  methods: {
    ...
    // 获取与当前code 关联的其它code(当当前code清除的数据删除的时候，重置关联的code)
    relationCode() {
      let codeArr = [];
      let _this = this;
      (function getCode(cmpts) {
        (cmpts || []).forEach(c => {
          if (c.location && c.location.code == _this.code) {
            codeArr.push(c.code)
          } else if (c.children) {
            c.children.forEach(ele => getCode(ele.components));
          }
        });
      })(this.formConfig||[]);
      return codeArr;
    ...
    changeHandle(e) {
      //用于监听清除数据,并重置关联的code
      let codeArr = this.relationCode();
      for(let i=0;i<codeArr.length;i++){
        let code = codeArr[i]
        if (this.data) {
          set(this.data, code,null);
        } else {
          set(this.formData.data,code,null);
        }
      }
      //用于监听清除数据,删除当前code
      if(!e.target.value){
