...
export default {
  name: "TableList",
  props: ["district", "orgid","dataSearchVal","executedVerbs"],
  components: {
    ARow: Row,
    ACol: Col,
    AInput: Input,
  ...
  watch: {
    district: {
      handler() {
        if(this.executedVerbs){
          this.executedverbIn = this.executedVerbs;
        }
        this.handlestatus = 0;
  ...
  methods: {
    getData(page) {
      ...
      let params = {
        ...
        executedverbIn: this.executedverbIn
      };
    ...
    onProgressFinish(res) {
      ...
      this.executedverbIn = [];
      keys.forEach(key => {
        this.executedverbIn.push({strategyid:key,version:executedVerbsMap[key]});
      });
      this.onSearch();
    },
