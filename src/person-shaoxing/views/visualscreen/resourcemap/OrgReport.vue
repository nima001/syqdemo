    ...
    <div class="cell" v-for="(group, index) in list" :key="index" :class="{'last':index == list.length-1}">
      <div class="cellItem" v-for="(item) in group" :key="item.title">
        ...
        <div class="num clickable" @click="showDetail(item)">
          <lcd-font :length="4" :realNumber="(orgInfo[item.total] - orgInfo[item.used]) | thanZero" :realStyle="item.realStyle"/>
        </div>
        <div class="type">
        ...
        <div class="header">
          <ul>
            <li>
              ...
              <lcd-font :length="4" :smooth="false" :realNumber="(detail.total - detail.used) | thanZero" :realStyle="{
                  color: detail.total < detail.used ? '#ff5500' : '#7dc8a3'
                }"/>
            </li>
            <li class="operater">=</li>
            <li>
              <div class="name">限额数</div>
              <lcd-font :length="4" :smooth="false" :realNumber="detail.total"/>
            </li>
            <li class="operater">-</li>
            <li>
              <div class="name">实有数</div>
              <lcd-font :length="4" :smooth="false" :realNumber="detail.used"/>
            </li>
          </ul>
        </div>
        <div class="body">
    ...
    return {
      ...
      list: [
        [
          ...
          { title: '局级行政机构剩余数', total: 'xzjjjg_xe+xzfjjjg_xe', used: 'xzjjjg_sy+xzfjjjg_sy' },
        ],
        [
          { title: '事业单位剩余数', total: 'syjg_xe', used: 'syjg_sy', realStyle: {color: '#ffd3a3'} },
          { title: '局级事业单位剩余数', total: 'syjjjg_xe+syfjjjg_xe', used: 'syjjjg_sy+syfjjjg_sy', realStyle: {color: '#ffd3a3'} },
        ]
      ],
      detail: {
        show: false,
      ...
      areaStatistics(district,[
        ...
        .then(({result}) => {
          ...
          this.list.forEach(item => {
            item.forEach(({total, used}) => {
              this.setValue(result, total, obj);
              this.setValue(result, used, obj)
            })
          })
          this.orgInfo = obj;
        })
            ...
            if(val == '剩余数'){
              return {
                ...
                content: data => {
                  let percent = '';
                  if(total > 0 && total >= used){
                    percent =  '，占比' + Number(((total - used) * 100/ total).toFixed(2)) + '%'
                  }
                  return `${title}${data.value}个${percent}`;
                },
                style: {
                  fill: '#fff',
                  fontSize: 16,
