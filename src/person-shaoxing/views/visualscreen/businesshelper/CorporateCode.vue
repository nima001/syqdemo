    ...
    <div class="cell">
      ...
      <div class="table">
        ...
        <template v-else>
          ...
          <ul class="tBody">
            <li v-for="(item,index) in list" :key="index" @click="showList(item.key)">
              <span>{{item.name}}</span>
              <span>{{item.v1}}</span>
              <span>{{item.v2}}</span>
            </li>
...
<script>
import {corporateCode,corporateorglist} from "@/person-shaoxing/api/orgStaffReport";
import { showError } from "@/framework/utils/index";
import SplitLine from "../components/SplitLine";
import DialogBox from "../components/DialogBox";
import Loading from "../components/Loading";
export default {
  components: { SplitLine, Loading, DialogBox },
  data() {
    return {
      list: [],
      keyCols: [],
  ...
  methods: {
    ...
    showList(key) {
      ...
      corporateorglist(key)
        .then(res => {
          this.changeLookup.list = res.result;
        })
        .catch(err => {
          showError(err);
        });
