  ...
  <div class="content">
    ...
    <div class="tablelist">
      <a-table
        ...
        :columns="columns"
      >
        <template slot="orgs" slot-scope="text">
          <a-tooltip>
            <template slot="title">
              <span class="orgs">{{text}}</span>
            </template>
            <span class="orgs">{{text}}</span>
          </a-tooltip>
        </template>
...
<script>
import { Form, Input, DatePicker, Button, Select, Modal, Table, Pagination, Tooltip } from "ant-design-vue";
import EditContent from "./components/EditContent";
import FileManage from '@framework/components/FileManage.vue';
import { includes, assign, map, cloneDeep } from "lodash";
      ...
      documentlist(this.pagination)
        .then((res) => {
          if (res.result.rows&&res.result.rows.length) {
            map(res.result.rows, (item)=>{
              item.orgs = map(item.orgs, 'orgname').join(',');
            });
