<template>
  <div class="uploadFile">
    ...
    <a-modal
      title="上传文件"
      :width="800"
      :bodyStyle="{ height: !record.id ? '570px' : '480px' }"
      :destroyOnClose="true"
      ...
      <a-form :form="form" :label-col="{ span: 4 }" :wrapper-col="{ span: 19 }">
        <a-form-item label="批准文号" :validateStatus="validateStatus" :help="help">
          <a-input-group :class="['pzwh', { disablepzwh: type===2 }]" compact>
            <a-input
              type="text"
              v-decorator="[ 'zihao', { initialValue: zihao, rules: [{ required: true, message: '请输入字号!' }], }, ]"
              style="width: 30%"
              @keyup="onChange"
            />
            <a-input
        ...
        <a-form-item label="文件上传" v-if="type===1">
          ...
          <ul class="fileList" v-if="this.fileList.length">
            <li v-for="item in fileList" :key="item.uid">
              <span :title="item.name">{{item.name}}</span>
              <span><a-icon type="delete" @click="remove"/></span>
            </li>
          </ul>
          <span class="fileList" v-else>未选择任何文件</span>
        </a-form-item>
