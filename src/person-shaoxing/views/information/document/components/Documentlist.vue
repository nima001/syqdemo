<template>
  <div class="Unfileddocument-query">
        ...
        >
          <template slot="action" slot-scope="text, record">
            <span v-if="active.id === 1">
              ...
              <span @click="previewPdf(record)">预览</span>
            </span>
    ...
    <preview-pdf v-model="pdfVisible" v-if="pdfVisible" :record="record"></preview-pdf>
  </div>
</template>
...
import { download ,uploadV2 ,getFileName} from "@/framework/api/file";
...
export default {
  ...
  methods: {
    previewPdf(record){
      getFileName(record.fileuri).then(res=>{
        if(decodeURI(res.result).endsWith('.pdf')){
          this.record = record;
          this.pdfVisible = true;
        }else{
          this.$notification.warning({
            message: "提示",
            description: "不是PDF文件不能预览!",
            duration: 1.5
          });  
        }
      }).catch(err=>{
