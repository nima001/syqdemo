...
export default {
  ...
  methods: {
    onOrgSelect(node, dept, init) {
      ...
      if(!org || org.orgtype == 3){//区域或分组显示报表
        this.$router.replace({path: '/person/org/report', query: {id: node.id, treeid: this.treeid}}).catch(() => {});
      }else{
        this.$router.replace({path: '/person/org/index', query: {id: node.id, treeid: this.treeid}}).catch(() => {});
      }
    },
  },
