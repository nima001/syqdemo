<template>
  <div class="table-list">
    ...
    <a-table
      ...
      <span slot="operation" class="operation" slot-scope="text, record">
        ...
        <template v-if="handlestatus == 2">
          <a @click="onRemoveAll(record.id)">取消黑名单</a>
          <a @click="onShowBlacklist(record.id)">查看详情</a>
        </template>
    ...
    >
      ...
      <black-detail :orgblackid="orgblackid"></black-detail>
    </a-modal>
...
import { problemPageList, orgBlacklist, problemblackList, addblackList, addDefer, removeBlacklist, batchremove, problemmonitorMsg, removeDefer } from "@/person-shaoxing/api/monitor";
...
export default {
        ...
        if(val) {
          if(val == 2) {
            ...
            this.initColumn = [
              ...
              { title: '关联问题数', dataIndex: 'problemnum', width:'20%' },
            ...
            this.initColumn = [
              { title: '类别', dataIndex: 'businesstype', customRender: this.dictRender('person.shaoxing.monitor.businesstype'), width:'10%' },
              { title: '名称', dataIndex: 'orgname', width:'15%' },
              { title: '问题或异动情况描述', dataIndex: 'description', width:'40%' },
              { title: '发现时间', dataIndex: 'discovertime', customRender: (text) => (text && text.substr(0, 10)), width:'7.5%' },       
              { title: '暂缓时间', dataIndex: 'defertime', customRender: (text) => (text && text.substr(0, 10)), width:'7.5%' },       
              { title: "操作", scopedSlots: { customRender: "operation" }, width:'20%' }
            ];
          }else{
            this.initColumn = [
              ...
              { title: '问题或异动情况描述', dataIndex: 'description', width:'50%' },
              { title: '发现时间', dataIndex: 'discovertime', customRender: (text) => (text && text.substr(0, 10)), width:'10%' },
              { title: "操作", scopedSlots: { customRender: "operation" }, width:'10%' },
            ];
          }
          ...
          this.initColumn = [
            ...
            { title: '问题或异动情况描述', dataIndex: 'description', width:'40%' },
            { title: '发现时间', dataIndex: 'discovertime', customRender: (text) => (text && text.substr(0, 10)), width:'7.5%' },
            { title: '状态', dataIndex: 'noticestatus', customRender: (text) => text == 0? '未通知': '已通知', width:'7.5%' },
            { title: "操作", scopedSlots: { customRender: "operation" }, width:'20%' }
          ];
          this.rowSelection = {
  ...
  methods: {
    getData(page) {
      ...
      if(this.handlestatus == 2 || this.handlestatus == 3){
