<template>
  <div class="staff-violate">
    <a-row :gutter="20">
      <a-col :span="24">
        <p class="section">
          <span class="section_content">（1）实有人员年龄分布情况</span>
        </p>
      </a-col>
      <a-col :span="24">
        <p class="section">
          <span class="section_content">30周岁以下人员<span>{{data.zeroth}}</span>名，占比<span>{{thirtyDown}}</span>；31周岁至40周岁人员<span>{{data.nineth}}</span>名，占比<span>{{thirtyToFourty}}</span>；41周岁至50周岁人员<span>{{data.eightth}}</span>名，占比<span>{{fourtyToFifty}}</span>；51周岁以上人员<span>{{data.seventh}}</span>名，占比<span>{{fiftyUp}}</span>；平均年龄 43.0 岁。</span>
        </p>
      </a-col>
      <a-col :span="24">
        <div id="ageChart"></div>
    ...
    <a-row :gutter="20">
      <a-col :span="24">
        <p class="section">
          <span class="section_content">（2）实有人员学历分布情况</span>
        </p>
      </a-col>
      <a-col :span="24">
        <p class="section">
          <span class="section_content">研究生以上学历人员<span>{{data.yjsxl}}</span>名，占比<span>{{yjs}}</span>;大学本科学历人员<span>{{data.dxxl}}</span>名，占比<span>{{dx}}</span>;大学专科学历人员<span>{{data.dzxl}}</span>名，占比<span>{{dz}}</span>;其他学历人员<span>{{data.qtxl}}</span>名，占比<span>{{qt}}</span>。</span>
        </p>
      </a-col>
      <a-col :span="24">
    ...
    <a-row :gutter="20">
      <a-col :span="24">
        <p class="section">
          <span class="section_content">（3）实有人员性别分布情况</span>
        </p>
      </a-col>
      <a-col :span="24">
        <p class="section">
          <span class="section_content">女性人员<span>{{data.women}}</span>名，占比<span>{{women}}</span>，男性人员<span>{{data.man}}</span>名，占比<span>{{man}}</span>。</span>
        </p>
      </a-col>
      <a-col :span="24">
        <div id="sexChart"></div>
...
import { numberToPercent, colorGroup, createHtmlTemp, pStyle } from "@/person-shaoxing/utils/index";
export default {
  props: {
    data: Object
  },
  ...
  methods: {
    getHtml() {
      let arr = [
        {val: `<p style="${pStyle}">（1）实有人员年龄分布情况</p>`},
        {val: `<p style="${pStyle}">30周岁以下人员${this.data.zeroth}名，占比${this.thirtyDown}；31周岁至40周岁人员${this.data.nineth}名，占比${this.thirtyToFourty}；41周岁至50周岁人员${this.data.eightth}名，占比${this.fourtyToFifty}；51周岁以上人员${this.data.seventh}名，占比${this.fiftyUp}；平均年龄 43.0 岁。</p>`},
        {},
        {val: `<p style="${pStyle}">（2）实有人员学历分布情况</p>`},
        {val: `<p style="${pStyle}">研究生以上学历人员${this.data.yjsxl}名，占比${this.yjs};大学本科学历人员${this.data.dxxl}名，占比${this.dx};大学专科学历人员${this.data.dzxl}名，占比${this.dz};其他学历人员${this.data.qtxl}名，占比${this.qt}。</p>`},
        {},
        {val: `<p style="${pStyle}">（3）实有人员性别分布情况</p>`},
        {val: `<p style="${pStyle}">女性人员${this.data.women}名，占比${this.women}，男性人员${this.data.man}名，占比${this.man}。</p>`},
        {}
      ]
