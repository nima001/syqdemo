      ...
      <a-affix :offset-top="0" :target="() => $refs.functionalBody" id="header-top">
        <div class="sticky">
          ...
          <div :class="['filter-content', { showfilter: filterVisible }]">
            ...
            <div :class="['filter-bottom', {'hide': !filterVisible}]">
        ...
        <div class="content-left">
          ...
          >
            <div class="header">
              ...
              <a-icon type="up" :class="{'rotate': !flag}" @click="changeArrow" />
            </div>
...
import { cloneDeep, has, includes, reduce } from "lodash";
...
export default {
  ...
  methods: {
    ...
    typeActive(val) {
      if (includes(this.choosetype, val)) {
        return true;
      }
      return false;
    },
    changeType(val) {
      if (includes(this.choosetype, val)) {
        let index = this.choosetype.indexOf(val);
        this.choosetype.splice(index,1);
      } else {
        this.choosetype.push(val);
      }
    },
    showFilter() {
    ...
    searchOrg() {
      if (this.searchkey == "") {
        ...
        let data = {
          ...
          type: reduce(this.choosetype,(sum,n)=> {return sum+n}),
        };
      ...
      await orgfuncdescSearch(list)
        .then((res) => {
          ...
          if (res.result.orgfuncdesc) {
            if((has(res.result.orgfuncdesc[0],'children')&&res.result.orgfuncdesc[0].children.length)||(has(res.result.orgfuncdesc[0], 'duties')&&res.result.orgfuncdesc[0].duties.length)) {
      ...
      .content-middle {
        ...
        .content {
          ...
          .under-mechanism {
            ...
            .yearreport:last-child, .dinggang ul:last-child li {
              border: 0;
            }
            .dinggang ul:not(:first-child) > span {
              margin-top: @layout-space-base;
            }
            .work-li {
              display: flex;
