...
export default {
  ...
  watch: {
    params: {
      ...
      handler(val) {
        if(val) {
          this.G6data = this.params.G6data;
          this.g6timedata = this.params.g6timedata;
          this.finddata = this.params.finddata;
          this.G6dataid = this.params.finddata.id;
          this.findMost(this.G6data.nodes);
          this.getmodeldata(this.finddata);
          let myDiv = document.querySelector(".small-div");
          if(myDiv&&this.$refs.container) {
            this.init();
          }
        }
      }
  ...
  methods: {
    ...
    init() {
      ...
      let color = finalStyle.background.split('none')[0]||finalStyle.getPropertyValue('background-color').split('none')[0];
      let that = this;
      if (that.plot) {
        that.plot.destroy();
      }
