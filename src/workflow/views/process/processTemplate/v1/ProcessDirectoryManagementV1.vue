  ...
  <a-layout class="processwrap">
    ...
    <a-modal :visible="edit" :title="type" @ok="editOk" @cancel="cancel">
      <a-form :form="editForm">
        ...
        <a-form-item
          v-if="(flag==1 || flag==4 ) && flowTypeFlag !== 9"
          label="发起类型"
          :label-col="{ span: 6 }"
          :wrapper-col="{ span: 12 }"
        >
        ...
        >
          <a-input
            v-decorator="['pcurl',{
               rules: [{ required:true, message:  `请输入关联的外部地址!` }],
            initialValue: formData.pcurl
          }]"
            placeholder="请输入关联的外部地址"
          ></a-input>
        </a-form-item>
      </a-form>
    </a-modal>
...
<script>
import { getListconstantbydictkey } from "@/workflow/api/workflow";
