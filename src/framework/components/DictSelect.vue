...
export default {
  ...
  props: {
    ...
    filter: {//数据过滤，Array.filter方法 或 显示的值数组
      type: [Function, Array],
    },
    showGroup: {//是否显示分组
      type: Boolean,
    ...
    selectList() {
      if(this.showGroup){
        ...
        this.enum.forEach((ele, index) => {
          ...
          if(this.filter && !this.doFilter(ele, index)){
            return;
          }
        ...
        return this.enum.filter(this.doFilter);
      }
      return this.enum;
    }
  },
  methods: {
    ...
    doFilter(ele, index){
      if(typeof this.filter == 'function'){
        return this.filter(ele, index);
      }else if(Array.isArray(this.filter)){
        return this.filter.indexOf(ele.value) >= 0;
      }
    },
