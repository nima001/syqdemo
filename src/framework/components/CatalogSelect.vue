...
export default {
  ...
  created() {
    this.resetSelect(this.selected);
    this.initData();
  },
  watch:{
    loadData(){
  ...
  methods: {
    ...
    async onCheckAll(){
      ...
      try {
        if(this.checkAll){//取消全选
          ...
          this.selectList = this.selectList.filter(item => map[item.id] !== undefined);
          this.dataList = [...this.dataList];
        }else{
    ...
    resetSelect(selected){
      ...
      if(!selected || !selected.length){//清空选择
        this.dataList.forEach(item => {
          if(item.checked){
            removed = true;
            item.checked = false;
      ...
      if(removed){//选择列表有移除，取消全选状态
        this.checkAll = false;
      }
      if(add || removed){//选择列表有变更刷新数据列表
        this.dataList = [...this.dataList];
      }
      this.$emit('select', this.selectList);
		},
