...
<script>
//FIXME sunwen 这里引入整个项目icons下的svg，require无法根据打包模块变量动态引入
const req = require.context('@', true, /\/assets\/icons\/.*\.svg$/);
req.keys().map(req);
// JSON.parse(process.env.VUE_APP_MODULES).forEach(m => {
//   const mReq = require.context(`@/${m.module}/assets/icons`, false, /\.svg$/);
//   mReq.keys().map(mReq);
//   if(m.plugin){
//     const pReq = require.context(`@/${m.module}-${m.plugin}/assets/icons`, false, /\.svg$/);
//     pReq.keys().map(pReq);
//   }
// });
 ...
 * 使用方式：将svg图标放入/[项目目录]/assets/icons文件夹下，type指定文件名即可（图标的映射关系可通过服务端配置修改）
 * Notes: 二次开发目录下放入同名的svg文件将覆盖原来的文件
 */
export default {
  name: "CustomIcon",
  props: {
