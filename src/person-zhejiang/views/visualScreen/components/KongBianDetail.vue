<template>
  <div class="detail-info">
    <div class="body">
      <div>
        <a class="back" v-if="isShow()&&(this.data.length||this.data.rows)" @click="back()">返回</a>
      </div>
      <ul class="bar-chart" v-if="data&&showXl">
    ...
    <dialog-box v-model="showOrgs" :title="title">
      <OrgList :district="districtcode" :jgtypes="[1, 2]" :systype="systype" />
    </dialog-box>
  </div>
</template>
<script>
      ...
      await query({...params}).then(({result}) => {
       if(result.rows.length) {
        data.forEach((item,index)=>{
          ...
          if(value.length) {
            data[index]['v'] = value[0]['_join0.xzjgs']||0;
          }
        });
        this.data = data;
       }
