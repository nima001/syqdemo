  ...
  <div class="layout">
    ...
    <div class="container">
      <SwiperChart>
        <div slot="content">
          ...
          <div class="chart-detail">
            <div class="chart-content" v-for="item in dataXz">
              <RingConsole class="chart-console" :data="item" :type="'dz'" :complietype="'a010101'" @showModal="showModal"/>
            </div>
          </div>
        ...
        <div slot="content">
          ...
          <div class="chart-detail">
            <div class="chart-content" v-for="item in dataZf">
              <RingConsole class="chart-console" :data="item" :type="'zf'" :complietype="'a010105'" @showModal="showModal"/>
            </div>
          </div>
        ...
        <div slot="content">
          ...
          <div class="chart-detail">
            <div class="chart-content" v-for="item in dataSy">
              <RingConsole class="chart-console" :data="item" :type="'sy'" :complietype="'a010103'" @showModal="showModal"/>
            </div>
          </div>
      ...
      <div class="warn">
        ...
        <div class="right" @click="()=>$router.push('/person/screen/problems')">
          <LcdFont :length="4" :realNumber="10" :realStyle="realStyle" :fakeStyle="fakeStyle"/>
        </div>
      </div>
    </div>
    
    <dialog-box v-model="showChart" :title="title">
      <chartDetail :districtCode="districtCode" :jgtype="jgtype" :dataList="dataList" :title="title" :type="type"/>
    </dialog-box>
    <dialog-box v-model="showAge" :title="title">
      <AgeDetail :districtCode="districtCode" :jgtype="jgtype" :showAge.sync="showAge" :data="dataSource" :title="title" :type="type"/>
    </dialog-box>
  </div>
</template>
...
import RingConsole from "./RingConsole";
...
export default {
  ...
  computed: {
    ...
    jgtype() {
      let v = this.$store.getters.dict('usermanage.org.jgtype');
      return v;
    },
  },
  mounted() {
    ...
    if(this.complietype&&this.districtCode) {
      this.loadData(this.districtCode);
  ...
  methods: {
    ...
    loadData(val) {
      Promise.all([
          chartAggregate(1097,{district: val, complietype: this.complietype[0].value}), 
          chartAggregate(1098,{district: val, complietype: this.complietype[0].value}),
        ]).then((result)=>{
          if(result[0]['code'] == "success" && result[1]['code'] == "success") {
            this.dataXz = [result[0].result.data,result[1].result.data];
          }
          Promise.all([
            chartAggregate(1097,{district: val, complietype: this.complietype[1].value}),
            chartAggregate(1098,{district: val, complietype: this.complietype[1].value}),
          ]).then((result)=>{
              if(result[0]['code'] == "success" && result[1]['code'] == "success") {
                this.dataZf = [result[0].result.data,result[1].result.data];
              }
              Promise.all([
                chartAggregate(1097,{district: val, complietype: this.complietype[2].value}),
                chartAggregate(1098,{district: val, complietype: this.complietype[2].value}),
              ]).then((result)=>{
                  if(result[0]['code'] == "success" && result[1]['code'] == "success") {
                    this.dataSy = [result[0].result.data,result[1].result.data];
                  }
              }).catch(error=>{
                showError(error);
              })
      ...
      // })
      // 政法
      // chartAggregate(1097,{district: val, complietype: this.complietype[1].value}).then(({result})=>{
      //   this.dataZf = [];
      //   this.dataZf.push(result.data);
      //   chartAggregate(1098,{district: val, complietype: this.complietype[1].value}).then(({result})=>{
      //     this.dataZf.push(result.data);
      //   }).catch(error=>{
      //     showError(error);
      //   })
    ...
    chartAgg(ids, data, title) {
      Promise.all([
          chartAggregate(ids[0],data),
          chartAggregate(ids[1],data)
        ]).then((result)=>{
          if(result[0]['code'] == "success" && result[1]['code'] == "success") {
            result[0].result.data.name = title[0];
            result[1].result.data.name = title[1];
            this.dataList = [result[0].result.data,result[1].result.data];
          }
      }).catch(error=>{
        showError(error);
      })
      // await chartAggregate(ids[0],data).then(({result})=>{
      //     result.data.name = title[0];
      //     this.dataList.push(result.data);
      //     chartAggregate(ids[1],data).then(({result})=>{
      //       result.data.name = title[1];
      //       this.dataList.push(result.data);
      //     }).catch(error=>{
      //       showError(error);
      //     })
...
.container {
  ...
  .warn {
    ...
    &::before {
      ...
      height: 100%;
      content: '';
      background:  url("../../../assets/img/screen/icon-problems-num-bg.png") no-repeat;
