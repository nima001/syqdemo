    ...
    >
      <a-form layout="horizontal" :form="form">
        <a-row :gutter="24" class="fatherbox">
          <a-form-item label="类别" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
            <DictSelect dict="library.documentsubtype" style="width: 100%" 
              v-decorator="['subtype', { rules: [{ required: true, message: '请填写类别' }] }]"
            />
          </a-form-item>
        </a-row>
        <a-row :gutter="24" class="fatherbox" v-if="1 === form.getFieldValue('subtype')">
          <a-form-item label="发文字号" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
            <a-input-group compact>
              <a-input
                type="text"
                style="width: 30%;"
                v-decorator="['num.name', { rules: [{ required: true, message: '请填写正确的发文字号' }] }]"
              />
              <a-input
                addonBefore="〔"
                addonAfter="〕"
                type="text"
                style="width:40%"
                v-decorator="['num.year', { rules: [{ required: true, message: '请填写正确的发文字号' }] }]"
              />
              <a-input
                addonAfter="号"
                type="text"
                style="width: 30%;"
                v-decorator="['num.ordinal', { rules: [{ required: true, message: '请填写正确的发文字号' }] }]"
              />
            </a-input-group>
          </a-form-item>
        </a-row>
        ...
        <a-row :gutter="24" class="fatherbox">
          <a-form-item label="文件分类" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
            <DictSelect dict="library.doctype" 
              v-decorator="['type', { rules: [{ required: false, message: '请选择文件类型' }] }]"
              :filter="item => item.key != 'taizhang' || form.getFieldValue('subtype') == 1"
              style="width: 100%"
            />
          </a-form-item>
        </a-row>
        <a-row :gutter="24" class="fatherbox">
          <a-form-item label="文件来源" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
            <DictSelect dict="library.doclevel" 
              v-decorator="['level']"
              style="width: 100%"
            />
          </a-form-item>
...
<script>
let id = 0;
import { Modal, Form, Row, Col, Input, Button, Select, Icon, DatePicker, } from "ant-design-vue";
    ...
    createdOk() {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          obj.dispatchdate = obj.dispatchdate && obj.dispatchdate.format("YYYY-MM-DD");
          if(obj.num){
            let { name, year, ordinal } = obj.num;
            obj.num = name + "〔" + year + "〕" + ordinal + "号";
          }
          obj.keywords = this.keywords;
          obj.orgs = this.nodelist;
          // 不上传文件类型就是待入库文件
          document(obj).then((res) => {
