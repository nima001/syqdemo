...
<template>
  <accordion-layout nav-title="组织架构">
    <org-tree class="org-tree" slot="nav" @select="onOrgSelect" :nodeid="nodeid" :treeid="treeid" />
    <keep-alive include="UserRosters">
      <router-view />
    </keep-alive>
  </accordion-layout>
</template>
...
export default {
  ...
  methods: {
    onOrgSelect(node) {
      if (this.$route.path == "/hall/usermanage/userinfo") {
        this.$router.push({ path: "/hall/usermanage/roster" });
      }
