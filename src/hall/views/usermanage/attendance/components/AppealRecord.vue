      ...
      <div class="middle">
        <a-table rowKey="id" :loading="loading" :customRow="customRow" :columns="columns" :data-source="dataSource" :pagination="false">
          <template slot="operate" slot-scope="text,record">
            <a @click="getUrl(record)" :class="{disabled:record.status==3}">{{record.status|optype}}</a>
          </template>
...
export default {
  ...
  filters:{
    ...
    toStr(v){
      ...
      switch (v){
        ...
        case 2 :str ='已驳回';break;
        case 3 :str ='已撤回';break;
        default:;break;
      }
      return str;
    },
  ...
  methods: {
    getUrl(record){
      formurl( `serialnum=${record.workflowid}` ).then(({result})=>{
         this.isUrl(result);
      }).catch(err=>{
        showError(err)
      })
    ...
    getData() {
      ...
      let query = {
        ...
        ...this.pagination,
        orders: [{orderby: "status","ordertype": "ASC"},{orderby: "checkdate","ordertype": "DESC"}]
