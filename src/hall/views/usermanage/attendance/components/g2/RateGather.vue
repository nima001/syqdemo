<template>
  <div class="content">
    <div class="middle">
      <a-table rowKey="orgname" :loading="loading" :customRow="customRow" :columns="columns" :data-source="dataSource" :pagination="false"></a-table>
    </div>
    <div class="bottom">
...
export default {
  ...
  methods: {
    getData() {
      this.loading = true;
      let query = {
        starttime: this.date ?moment(this.date).startOf('month').format("YYYY-MM-DD") : undefined,
        endtime:this.date ? moment(this.date).endOf('month').format("YYYY-MM-DD"):undefined,
        ...this.pagination
      };
      rate(query)
        .then(res => {
