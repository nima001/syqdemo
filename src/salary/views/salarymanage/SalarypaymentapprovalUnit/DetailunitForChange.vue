            ...
            <div class="audit">
              ...
              >
                <span slot="operate" slot-scope="val,record">
                  <span v-if="!record.itemid" @click="showChange(1,record,0)" class="spancss">调整比例</span>
                  <span
                    v-if="record.itemid && !record.salaryscale"
                    @click="showChangePay(record)"
                    class="spancss"
                  >调整金额</span>
                  <span
                    v-if="record.itemid && record.salaryscale"
...
import { Layout, Breadcrumb, Table, Tooltip, Icon } from "ant-design-vue";
...
export default {
  ...
  created() {
    ...
    usersalary(data)
      .then((data) => {
        ...
        that.personInfo.birthday = data.result.birthday.slice(0, 11);
        //请求性别
        ...
        constantdictlist("usermanage.user.identitytype")
          .then((data) => {
            ...
            that.personInfo.identitytype = obj.text;
          })
          .catch((err) => {
            showError(err);
          });
        ...
        constantdictlist("usermanage.user.treatlevel")
          .then((data) => {
            ...
            that.personInfo.treatlevel = obj.text;
          })
          .catch((err) => {
            showError(err);
          });
        ...
        constantdictlist("usermanage.user.salarylevel")
          .then((data) => {
            ...
            that.personInfo.salarylevel = obj.text;
          })
          .catch((err) => {
            showError(err);
          });
  ...
  methods: {
    ...
    showChange(isUser, record, type) {
      if (isUser == "userSelfChange") {
        ...
        if (type == 0) {
          ...
          salaryupdaterecord(payroll)
            .then((data) => {
              record.changeHistory = data.result; //获取调整记录
              this.record = record;
            })
            .catch((err) => {
              showError(err);
    ...
    reget() {
      ...
      processdetail(data)
        .then(function (res) {
          let salarydetail = res.result; //数据

          that.tableOne = salarydetail;

          //第一张表
          for (let i = 0; i < that.tableOne.result.length; i++) {
            that.tableOne.result[i]["key"] = i + 1;
            let childrenArr = that.tableOne.result[i].children;
          ...
          that.datasourceone = that.tableOne.result;
        })
        .catch((err) => {
          showError(err);
        });
