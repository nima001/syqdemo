;(function () {
  var Prompt = {
    ...
    init: function (option) {
      ...
      var shade = (this.shade = option.shade || false)
      // 是否显示遮罩
      var opacity = (this.opacity = option.opacity || 20)
      // 遮罩透明度
      var width = (this.width = option.width || 500)
      var height = (this.height = option.height || 300)
      var _temp = (this._temp = option.html || '')
      var ConfirmFun = (this.ConfirmFun = option.ConfirmFun || false)
      var CancelFun = (this.CancelFun = option.CancelFun || false)
      this.editTitle()
      this.editHtml()
      if (ConfirmFun) {
        this.showBottom()
    ...
    createPrompt: function () {
      ...
      Div.id = 'prompt'
      Div.innerHTML =
        "<span id='prompt_close'></span><div id='prompt_title'></div><div id='prompt_body'></div><div id='prompt_bottom'></div>"
    ...
    showBottom: function () {
      // 创建确定 取消按钮
      var that = this
      var prompt_bottom = that.$('prompt_bottom')
      if (that.CancelFun) {
        prompt_bottom.innerHTML =
          "<button class='btn' id='ConfirmFun'>确定</button>"
