...
ORYX.Plugins.RenameShapes = Clazz.extend({
	...
	actOnDBLClick: function(evt, shape){
        ...
        //节点类型判断
        var namespace = shape.getStencil().namespace();//命名空间
        var shapeType = shape.getStencil().id();//点击的图形类型
        var url = "";
        var parent = this;
        var title = "属性配置";
        var oldvalue = this.oldValueText;//节点名称老的值
        var resourceId = shape.resourceId;//节点id
        var modelInstanceId = EDITOR.UTIL.getParameterByName("modelInstanceId");
        ...
        //用户任务节点/结束节点
		var targetNodes = [];
        if((shapeType === namespace+"UserTask") || (shapeType === namespace+"EndNoneEvent")){
            url = "../nodeconfig/usertask";
        }else if(shapeType === namespace+"ExclusiveGateway"){
            //单一网关节点
            url = "../nodeconfig/getway";
            //获取网关节点连接的目标节点
            var outgoingShaps = shape.getOutgoingShapes();//网关节点的出口连线
            if(outgoingShaps != undefined && outgoingShaps.length > 0){
                jQuery.each(outgoingShaps,function (k,v) {
                    var targetNode = v.getOutgoingShapes();//网关节点出口连线的目标节点
                    if(targetNode != undefined && targetNode.length > 0){
                        jQuery.each(targetNode,function (n,m) {
                            var node = {
                                "resourceid":v.resourceId,//设置节点id为连线的节点id
                                "name":m.properties["oryx-name"]
                            };
        ...
        }else if(shapeType === namespace+"CallActivity"){
			url = "../nodeconfig/callactivity";
		}

        //双击前保存流程图json
        jQuery.ajaxSetup({async:false});
        var serializeJson = this.facade.getSerializedJSON();
        // jQuery.post(window.parent.getMyVue.apiUrl+"/modelinfo/savejson",{"modelId":modelId,"json_xml":serializeJson},function(r){});
        jQuery.ajaxSetup({async:true});

        if(url){
			//开始节点/用户任务节点/结束节点
			var  isendnode = 0;
			if(shapeType === namespace+"EndNoneEvent"){
                isendnode = 1;
			}
			url += "?modelInstanceId="+modelInstanceId+"&modelId="+modelId+"&resourceId=" + resourceId+"&isendnode="+isendnode+"&targetNodes="+JSON.stringify(targetNodes);
			//判断节点类型给vue页面传递数据
			if(shapeType === namespace+"UserTask"){
		...
		new function(){
			window.addEventListener('message', function(e){
				if(e.data.data !== oldvalue){
					parent.updateValueFunction(e.data.data,oldvalue);
				}
		  })
		  that.destroy();
		}();	
