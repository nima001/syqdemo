    ...
    >
      <span slot="operation" class="operation" slot-scope="text, record">
        ...
        <template v-if="handlestatus == 2">
          <a @click="onRemoveAll(record.id)"
            >取消黑名单</a
          >
          <a @click="onShowBlacklist(record.id)"
            >查看详情</a
          >
        </template>
...
<script>
import {Row, Col,Input,Table,Pagination,Button,Modal,Select,Icon} from "ant-design-vue";
...
export default {
  name: "TableList",
  props: ["district", "orgid", "title","dataSearchVal","cityId"],
  components: {
    ARow: Row,
    ACol: Col,
    AInput: Input,
  ...
  watch: {
    ...
    title: {
      handler(val) {
        ...
        console.log(this.handlestatus,val);
      },
      immediate: true,
    },
    handlestatus: {
        ...
        if (val) {
          if (val == 2) {
            ...
            this.initColumn = [
              ...
              { title: "名称", dataIndex: "orgname", customRender: text => <span title={text}>{text}</span>,width: "15%" },
              {
                title: "问题或异动情况描述",
  ...
  methods: {
    getData(page) {
      ...
      console.log(this.handlestatus);
      if(this.handlestatus === 0 || this.handlestatus === 2 || this.handlestatus === 3 || this.handlestatus === 5){
