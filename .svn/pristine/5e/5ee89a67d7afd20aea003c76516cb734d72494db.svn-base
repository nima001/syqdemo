<template>
  <div>
    <div id="preloadingAnimation" ref="animate">
...
import {loginByTicket} from '@/login/api/login'
...
export default {
  ...
  methods: {
    login() {
      ...
      let ticket = search.get('ticket');
      let sp = search.get('sp');
      loginByTicket(ticket)
        .then(res => {
          Bus.$emit("afterLogin", res.result);
          if(sp){
