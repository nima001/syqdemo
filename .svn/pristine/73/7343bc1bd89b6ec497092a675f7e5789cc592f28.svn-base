(function(window){
	...
	var configs = {
		...
		models:{//模块
			...
			suggest:function(){return idm.url+'/static/js/yh-suggest.js'}
		},
		encrypturl:function(){return idm.url+'/static/js/jsencrypt.js'},
		valisignurl:function(){return idm.url+'/js51/valisign'},//签名验证
		ticketloginurl:function(){return idm.url+'/js51/ticketlogin'},
		loginurl:function(){return idm.url+'/js51/login'},
		isloginurl:function(){return idm.url+'/js51/islogin'},
		logouturl:function(){return idm.url+'/js51/logout'},
		infourl:function(){return idm.url+'/js51/userinfo'},
		
		openauthloginurl:function(){return idm.url+'/js51/jsauth'},
			...
			idm.callback.before = function(issucc,resJson){
				if(issucc){
					...
					if(resJson && resJson.encode){//需要rsa加密
						...
						idm.load.addScript(configs.encrypturl(), function(){
							var instance = new JSEncrypt();
							instance.setPublicKey(idm.encrypt.publickey);
							idm.encrypt.instance = instance;
							resolve();
						});
	...
	idm.callback = {
		...
		call:function(issuccess,jsonRes){//回调交互总入口
			...
			var cb = idm.callback.before(issuccess,jsonRes);
			var resolve = function(){
				if(issuccess){
					if(jsonRes.appToken){
						setCookie("appToken",jsonRes.appToken,8);
					}
					if(jsonRes.unifylogin){
						window.open(configs.openauthloginurl()+"?servicecode="+configs.servicecode+"&appToken="+jsonRes.appToken,"authlogin","width=1050,height=800");
					}
	...
	idm.user={//用户相关对外接口
		...
		ticketlogin: function(options){//票据登录
			...
			var unifylogin = false;
			if(options && options.unifylogin){
				unifylogin = true;
			}
			if(options && options.ticket){
				url = url + "?servicecode="+configs.servicecode+"&time="+options.time+"&sign="+options.sign+"&domain="+document.domain
						+"&ticket="+options.ticket+"&unifylogin="+unifylogin;
			}
			idm.ajaxRequest(url);
		},
		login: function(options){//票据登录
			...
			url = url  + "?servicecode="+configs.servicecode+"&time="+options.time+"&sign="+options.sign+"&domain="+document.domain
				+"&loginname="+encodeURIComponent(options.loginname)+"&password="+encodeURIComponent(password)+"&getticket="+getticket+"&unifylogin="+unifylogin;
			idm.ajaxRequest(url);
		},
		...
		islogin: function(options){//票据登录
			...
			if(!appToken){
				idm.callback.call(false,{code:'is_not_login',msg:'账号未登录,请登录'});
				return ;
			}
			//异步ajax请求
		...
		logout: function(options){//登出
			...
			//回调之前
			idm.callback.before = function(issucc,resJson){
				deleteCookie("appToken");//删除本地cookie(无论异地登出状况,本地均实行登出)
				if(issucc){
					if(resJson && resJson.logouturls){//需要rsa加密
						for(var v in resJson.logouturls){
							idm.load.addScript((resJson.logouturls)[v]);
						}
			...
			//异步ajax请求
			var url = configs.logouturl()+ "?servicecode="+ configs.servicecode + "&appToken=" + appToken;
			idm.ajaxRequest(url);
		},
	...
	idm.load = {
		...
		addScript:function(src, callback){
			...
			if(callback){
				(script.readyState ? function(node, callback) {
