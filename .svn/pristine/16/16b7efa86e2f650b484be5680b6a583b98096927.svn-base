    ...
    orgtermSearch(){
      ...
      orgtermsearch(this.pagination).then((res)=>{
        ...
        res.result.rows.forEach((item,index)=>{
          res.result.rows[index].name=this.brightenKeyword(this.transformationContent(item.name||''));
          res.result.rows[index].desc=this.brightenKeyword(this.transformationContent(item.desc||''));
        });
