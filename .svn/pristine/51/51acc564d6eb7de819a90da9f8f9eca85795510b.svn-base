        ...
        <div v-for="(item, index) in pagination.rows" :key="index">
          <div class="list-content">
            ...
            <span class="time-span">
              <span v-if="item.num">批准文号：{{ item.num }}</span>
              <span v-if="item.dispatchdate">发文时间：{{ item.dispatchdate }}</span>
              <span v-if="item.type" v-html="getType(item.type)"></span>
              <span v-if="item.orgs.length">相关单位：
...
export default {
  ...
  methods: {
    getType(type) {
      let array = this.doctype.filter((item)=>item.value==type);
      if(endsWith(array[0].text,'文件')) {
        return `文件类型：${array[0].text}`;
      }else{
        return `文件类型：${array[0].text}文件`;
      }
    },
    //判断搜索记录是否包含某个关键字
        ...
        .box {
          ...
          .in-box {
            ...
            .li-content {
              ...
              &:first-child {
                width: 320px;
                /deep/.ant-input-group-wrapper .ant-input:nth-child(2) {
                  width: 55px;
                  text-align: center;
                }
              }
