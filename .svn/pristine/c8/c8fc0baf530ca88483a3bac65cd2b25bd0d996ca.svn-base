<template>
  <div class="carousel-container" :class="{loading: initTimer}" ref="container">
    <slot />
  </div>
</template>
<script>
...
export default {
  props: {
    ...
    autoPlay: {// [ 0: off | number 指定切换步长 (整数顺时针，负数逆时针) ]
      type: Number,
      default: 0
    },          
    autoPlayDelay: {
  ...
  methods: {
    ...
    onClick(event){
      let container = this.$refs.container, 
        domPath = event.path || (event.composedPath && event.composedPath());
