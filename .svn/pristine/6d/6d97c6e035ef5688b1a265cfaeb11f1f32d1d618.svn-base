      ...
      <div class="content">
        ...
        >
          <p style="margin: 0" slot="orgname" slot-scope="text, record">
            <a-tooltip>
              <template slot="title">
                <span v-for="(item, index) in record.orgs" :key="item.orgid">
                  <span v-if="record.orgs.length !== index + 1"> {{ item.orgname + "，" }} </span>
          ...
          <a-form-item label="组成结构">
            <a-input
              ...
              v-decorator="[
                ...
                {
                  initialValue:
                    this.nodelist.length > 0 ? this.nodelist.length : undefined,
...
import { orglinequery, orglinesave, orglinedel, orgvalid } from "@/person-shaoxing/api/information";
...
export default {
  ...
  methods: {
    //校验是否重复
    async isRepeat(rule, value, callback) {
      await orgvalid({
        name: value,
        orgs: this.records.orgs,
        id: this.records.id,
      })
        .then((res) => {
          callback();
