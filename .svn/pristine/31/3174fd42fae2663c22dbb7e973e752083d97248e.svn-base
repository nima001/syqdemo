<template>
  <div class="examine">
    ...
    >
      <QuestionList :data="questionList"></QuestionList>
    </a-modal>
  </div>
</template>

<script>
import { Button, Upload, Table, Divider, Tabs, Icon, Pagination, Select, Modal } from "ant-design-vue";
import { getexamine, getuserinfo, uploaduserinfo, exportexamine } from '@/person/api/examineManage';
import { uiConfigsCookies } from '@/framework/utils/auth';
import { showError } from "@/framework/utils/index";
import QuestionList from '@/person-yuhangmz/views/exam/QuestionList';
    ...
    getData(orgid, nodeid, year) {
      if (this.loadData && this.loadData.node && this.loadData.node.data) {
        ...
        .then(data => {
          ...
          if(!year) {
            this.currentYear = res.year ? res.year : year;
          }
