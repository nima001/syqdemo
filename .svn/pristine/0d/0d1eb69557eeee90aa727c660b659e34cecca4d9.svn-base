...
export function openZwccInfo(geocode, sfsbj, zwjb, jgType){
  getSMZParams().then(({userId, serviceCode}) => {
    window.open(`http://59.202.42.184:8008/api.smz.do?m=getGwyByZwjbList&geocode=${geocode}&sfsbj=${sfsbj}&zwjb=${zwjb}&jgType=${jgType}&serviceCode=${serviceCode}&userId=${userId}`);
  }).catch(error => {
    console.log(error);
  })
}
