  ...
  <div class="layout">
    <div class="content">
      <div class="top">
        <div>
          <template v-if="!isOrg">
            <span class="title">加班人数:<span class="num">{{counts.number || 0}}</span></span>
            <span class="title">加班次数:<span class="num">{{counts.addworkTime || 0}}</span></span>
          </template>
    ...
    <count-info v-if="showModal" v-model="showModal" :user="user"></count-info>
  </div>
</template>
<script>
...
import {workByOrg, workByUser,worklist} from "@/hall/api/workovertime";
