<template>
  <div v-cloak>
    ...
    >
      ...
      <a-table :columns="batchSignColumns" :pagination="batchStampProcesspagination" :dataSource="batchStampdata" class="thinkbank-table"
      :rowSelection="{selectedRowKeys: batchkey, onChange: changebatchData}" :scroll={y:400,x:790}>
        <template slot="operate1" slot-scope="text,record">
              <span @click="openBatchStamp(record)" style="color:#3990fa;cursor:pointer;">查看</span>
            </template>
      </a-table>
...
export default {
    ...
    return {
      ...
      batchSignColumns:[
        ...
        },
        {
            ...
            key: "operate1",
            width: 120,
            scopedSlots: { customRender: "operate1" }
          }
      ],
  ...
  methods: {
    ...
    changebatchsign(page, pageSize) {
      this.batchkey = [];
      this.getbatchStampProcess(page);
    },
    //确定选择的机构
    orgOk(type, list) {
    ...
    saveStamp() {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          let arr = [];
          this.batchkey.forEach(a => {
            arr.push({
                  businessInstanceId: this.batchStampdata[a].businessinstanceid,
                  taskId: this.batchStampdata[a].taskid,
                  modelInstanceId: this.batchStampdata[a].modelinstanceid
             });
          });
    ...
    changebatchData(selectedRowKeys) {
      this.batchkey = selectedRowKeys;
    },
    batchSigning() {
