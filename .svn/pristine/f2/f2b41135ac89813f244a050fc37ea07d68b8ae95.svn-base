<template>
  <div>
    <a-modal :title="title" :width="1060" :visible='true' :destroyOnClose='true' @cancel="onFinish('cancel')">
      ...
      <textarea class="ant-input" id="textArea" ref="textArea" 
        v-model="params" 
        v-focus
        @keydown.tab="onTabInput"
        :rows="4" 
        placeholder="请输入表达式"
        :style="{resize:'none'}"
      ></textarea>
      <a-row :gutter="8">
        ...
        <a-col :span="8" v-if="havaDataSource">
          <data-source 
            v-model="params" :dom="dom" 
            :namespace="dataNs || namespace" 
            :selfFields="selfFields"
            :dataSource="dataSource" 
            @sourceCall="sourceUpdate"
          />
        </a-col>
      </a-row>
    </a-modal>
...
export default {
  ...
  methods: {
    ...
    onTabInput(e){
      e.preventDefault();
      let start = this.dom.selectionStart, end = this.dom.selectionEnd;
      let text = this.params || '';
      this.params = text.substr(0, start) + '\t' + text.substr(end);
      this.$nextTick(() => {
        this.dom.setSelectionRange(start + 1, start + 1);
      })
    }
  }
};
