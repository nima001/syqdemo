...
import { treepath, treeroot, treequery, listnode, orgquery } from "@/hall/api/usermanage";
...
export default {
  ...
  created(){
    Bus.$on('update', ()=>{
      this.$refs.tree.initData && this.$refs.tree.initData();
    }); 
  },
  computed:{
    unittypes(){
  ...
  methods: {
      ...
      if(this.showDept){
        ...
        if(parent){
          return orgquery({
            ...
            return (result.rows || []).map(item => ({ id: item._id, name: item.name, type: 'org', data: item }))
          });
        }
      }
    },
    ...
    onSelected(node, extra, init){
      this.$emit('select', node, extra && extra.data, init);
    },
    checkChild(list){
      if(list){
        list.forEach(item => {
          item.type = item.data ? 'org' : 'dir';
