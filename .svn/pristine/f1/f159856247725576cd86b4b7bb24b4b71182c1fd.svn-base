  ...
  <div class="layout">
    <div class="content">
      <div class="top">
        <div>
          <template v-if="!isOrg">
            <span class="title">考勤人数:<span class="num">{{counts.usernum || 0}}</span></span>
            <span class="title">考勤异常次数:<span class="num">{{counts.abnormalnum || 0}}</span></span>
          </template>
          <a v-if="isOrg" @click="$emit('finish',{cmpt:'DepartmentList'})">返回上一级</a>
        </div>
        <div>
      ...
      <div class="middle">
        <a-table rowKey="userid" :loading="loading" :customRow="customRow" :columns="columns" :data-source="dataSource" :pagination="false"></a-table>
      </div>
      <div class="bottom">
    ...
    <count-info v-if="showModal" v-model="showModal" :user="user" :formatDate="query.date"></count-info>
  </div>
</template>
<script>
...
import {usersumError,orgsumError} from "@/hall/api/attendance";
