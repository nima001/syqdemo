<template>
  <div class="impower-manage">
    ...
    <div>
    <a-modal destroyOnClose title="添加角色" :footer="null" v-model="addvisible"
      width="1000px" :bodyStyle="{height: '520px', padding: 0, overflow: 'hidden'}">
      <add-role :analyzeid="analyzeid" @finish="onAddRole"></add-role>
    </a-modal>
    </div>
  </div>
</template>
...
import { listRole, delRole } from "@/person/api/statistics";
export default {
  name: 'impowerManage',
  data() {
    return {
  ...
  created() {
    this.analyzeid = this.$route.query.id;
    this.loadData();
  },
  methods: {
    loadData() {
      this.loading = true;
      listRole(this.analyzeid)
      .then(({result}) => {
