    ...
    <header class="header">
      ...
      <div class="header-middle">
        <template v-for="item in topApps">
          <a-popover v-if="item.children && item.children.length" 
            :mouseEnterDelay="0.4"
            :key="item.id"
          >
            <table slot="content" class="sub-menus">
              <tr v-for="(group,index) in groupList(item.children)" :key="index">
                <td v-for="(menu, i) in group" @click="jump(menu)" :class="{item: !!menu}" :key="i">{{menu && menu.name}}</td>
              </tr>
            </table>
            <a :class="{active: isActive(item.componenturi)}" >{{item.name}}</a>
          </a-popover>
          <a v-else
            @click="jump(item)" 
        ...
        >
          ...
          <a @click="toBusinessCenter" :class="{active: isActive('/main/menu')}">业务中心</a>
        </a-popover>
      </div>
      <div class="header-right">
        <template v-if="session">
          ...
          <a-badge :count="notification.totalnum || 0" dot class="item">
            <router-link to="/main/message"><a-icon class="bell" type="bell"/></router-link>
          </a-badge>
          <!-- <a-icon type="user" /> -->
...
export default {
  ...
  computed: {
    ...
    appList(){//应用列表
      return (this.$store.getters.menuList || []).filter((item) => ((item.flag || 0) & (1 << 2)) == 0);
    },
    topApps(){//放在顶部的菜单列表
    ...
    notification(n){
      if(n.count > 0){//提醒新消息
        this.$notification.open({
          key: 'system-message-notification',
          message: '新消息提醒',
          description: `您收到${n.count}条新消息，点击查看详情`,
          icon: <a-icon type="message" style="color: #52c41a"/>,
          duration: 5,
          onClick: () => {
            this.$notification.close('system-message-notification');
  ...
  methods: {
    isActive(path, precise){
      ...
      return !precise && routePath.startsWith(path + '/') || routePath == path;
    },
    ...
    groupList(list){
      let count = 5, size = Math.ceil(list.length / count);
      if(size){
