...
import { has, cloneDeep, map, concat } from "lodash";
      ...
      return {
        ...
        onChange: (selectedRowKeys, selectedRows) => {
          ...
          selectedRows.forEach((item, index)=>{
            let { title, annexuri, fileuri } = item;
            if(fileuri) {
              this.annexList.push({ annexname:title, annexuri:annexuri});
            }else{
              notify = true;
            }
          });
  ...
  methods: {
    customRow(record,index) {
      return {
        on: {
          click: (e)=>{
            if(record&&record.annexuri) {
              let rowKeys=this.selectedKeys;
              if(rowKeys.length>0 && rowKeys.includes(record.annexuri)){
                rowKeys.splice(rowKeys.indexOf(record.annexuri),1);
                let index = this.annexList.findIndex(item=> item.annexuri==record.annexuri);
                this.annexList.splice( index, 1);
              }else{
                rowKeys.push(record.annexuri);
                this.annexList.push({ annexname:record.title, annexuri:record.annexuri})
              }
    ...
    showSelect() {
      this.selectedKeys = map(this.annexList, 'annexuri');//打开弹窗默认勾选
      this.show = true;
      if(!this.data.length) {
        this.documentList();
      }
    ...
    modalOk() {
      this.selectedKeys = map(this.annexList, 'annexuri');
      this.show = false;
    },
      ...
      documentlist({pagenum: current,pageSize, ...this.searchKey})
        .then((res) => {
          if (res.result.rows&&res.result.rows.length) {
            ...
            map(rows, (item)=>{
              if(item.fileuri) {
                let str = item.fileuri.split('file://')[1];
