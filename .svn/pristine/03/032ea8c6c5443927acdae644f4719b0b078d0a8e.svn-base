  ...
  <div class="container" style="height: 500px">
    <div class="content">
      <a-tabs default-active-key="1" @change="callback">
        <a-tab-pane key="1" tab="我关注的" force-render v-if="hasPermit('/person/worktask/concern')">
          <a-table
            rowKey="id"
            :pagination="false"
            :loading="spinning"
          ...
          >
            ...
            <template slot="operation" slot-scope="text,record">
              <span @click="record.flowtype?deal(record):worktaskDetails(record)">查看</span>
            </template>
          </a-table>
          <a-pagination
        ...
        <a-tab-pane key="2" tab="我要办的" v-if="hasPermit('/person/worktask/work')">
          <a-table
            rowKey="id"
            :pagination="false"
            :loading="spinning"
      ...
      <div class="option">
        <a-select @change="handleChange" placeholder="请选择任务类型" style="width: 160px;margin-right: 10px" allowClear>
          <a-select-option v-for="item in tasktypeList" :key="item.key" :value="item.value">
            {{item.text}}
          </a-select-option>
        </a-select>
        <a-button type="primary" ghost @click="toPublish" v-if="hasPermit('/person/worktask/publish')">发布任务</a-button>
      </div>
    </div>
    <!--详情弹窗-->
 ...
 * 工作任务
 */
import { Tabs, Table, Button, Pagination, Modal, Row, Col, Select } from "ant-design-vue";
...
export default {
    ...
    return {
      ...
      attentioncolumns: [
        ...
        },
        {
          ...
          customRender: this.dictRender("person.work.taskcategory"),
          width: '18%'
        },
        {
          title: "发布人",
  ...
  methods: {
    handleChange(value) {
      this.pagination.tasktype = value;
