  ...
  actions: {
    loadConfig(context, reload){
      if(context.state.config && !reload){
        ...
        let configUrl = process.env.VUE_APP_CONFIG_UI_URL || '/api/config/v1/config/ui/' + process.env.VUE_APP_PROJECT_NAME;
        // let configUrl = '/idmgzmh/acl/menu/getFrontData';
        loader = axios.get(configUrl)
        .then(response => {
          loader = undefined;
          let res = response.data;
          if (res.code == 'success') {
            // let uiConfigs = Object.fromEntries((res.result || []).map(item => [item.key, item.value]))
            let uiConfigs = ObjectfromEntries(res.result);
            //根据配置修改最终的接口地址
            uiConfigs['api.url'] = uiConfigs['api.url'] || '/api';
