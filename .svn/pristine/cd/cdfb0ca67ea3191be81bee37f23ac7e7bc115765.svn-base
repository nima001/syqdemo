<template>
  <div class="booklet-change">
    <a-form class="book-form" :form="form">
      ...
      <a-row :gutter="24">
        <a-col :span="12">
          <a-form-item label="分类">
            <dict-select dict="report.booknamespace"
      ...
      <a-row :gutter="24">
        <a-col :span="12">
          <a-form-item label="归档方式">
            <a-select 
              allowClear  
              v-decorator="['archive', { rules: [{ required: true, message: '请选择归档方式'}] }]"
              >
              <a-select-option value="none">不归档</a-select-option>
              <a-select-option value="month">按月归档</a-select-option>
              <a-select-option value="quarter">按季度归档</a-select-option>
              <a-select-option value="year">按年归档</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label="相关单位">
            <a-input
              :read-only="true"
              :value="bookInfo && bookInfo.orgname"
              allowClear
              @click="showOrgSelect"
            >
              <a-icon v-if="bookInfo && bookInfo.orgid" slot="suffix" class="clear-org"
                theme="filled" type="close-circle" @click="clearOrg()"
              />
      ...
      <PropValueForm v-if="contextProps && contextProps.length"
  ...
  methods: {
    ...
    loadBookinfo() {
      ...
      .then(({result}) => {
        ...
        this.contextProps = result.fields;
        this.contextData = this.bookInfo.context;
        this.$nextTick(() => {
      ...
      return new Promise((resolve, reject) => {
        this.form.validateFields((error, values) => {
          if(error){
            ...
            values = Object.assign({}, this.bookInfo, values);

            let contextFrom = this.$refs.contextFrom;
            if(contextFrom){
              contextFrom.getFieldsValue().then((context) => {
    ...
    orgOk(type, list) {
      ...
      if (type == "ok" && list.length > 0) {
        let { _id, name } = list[0];
        this.bookInfo.orgid = _id;
        this.bookInfo.orgname = name;
      }
    },
    tempChange(value) {
      ...
      if(temp){
        this.contextData = this.bookInfo && this.bookInfo.context;
        this.contextProps = temp.fields;
      }
    },
    onScroll(e) {
    ...
    clearOrg(){
      this.bookInfo.orgid = undefined;
      this.bookInfo.orgname = undefined;
    }
  }
}
</script>
