<template>
  <a-spin :spinning="!!loadid" :delay="200">
    <div class='chart-panel'>
      <component v-if="tableData" :is="chartType" :data="tableData" :settings="settings" ref="chart"/>
...
export default {
  ...
  methods: {
    initData(){
      // console.log('initData', this.name, this.chart);
      if(typeof this.chart == 'number'){
        ...
        this.chartType = camelCase(settings.chartType);
      }else{
    ...
    loadData(id){
      ...
      this.loadid = id;
      // console.log('loadData', id, this.loadid);
    ...
    //     case "stack-bar-chart" :cmptType = "StackBarChart";break;
    //   }
    //   return cmptType;
    // },
    validate(callback) {
      if(this.propValue && this.propValue.data){//数据已经加载，propValue值变更会触发validate
