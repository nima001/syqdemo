...
import  * as orgApi from '@/person/api/org'
import {  deptuserquery } from '@/person/api/user'
  ...
  methods: {
      ...
      if(this.selected){
        if(this.mode == 'org' || this.mode == 'dept'){
          return this.selected.map(item => {
            return { id: item._id, name: item.name, data: item };
          })
        }else if(this.mode == 'user'){
          return this.selected.map(item => {
            return { id: item._id, name: item.username, data: item };
          })
        }else{
          return this.selected.map(item => {
    ...
    onFinish(type){
      ...
      if(this.mode == 'orgtree' ){
        list = this.selectList.map(item => ({id: item.id, name: item.name}));
      }else{
        list = this.selectList.map(item => item.data);
      }
      if(type == 'ok'){
    ...
    getRoot(){
      if(this.defaultRoot){//
        return this.orgFn('treequery')({ 
          allsub: true, 
          pagesize: 1, 
          dataid: this.defaultRoot 
        }).then(({result}) => {
          let list = this.convertTreeNodeList(result.rows, true);
          if(list.length > 0){
            if(list.length > 1){
              console.warn('指定defaultRoot的单位对应多个节点，暂只支持单个节点展示');
            }
            this.defaultRootNodeId = list[0].nodeid;
          }
          return { id: 0, name: '组织架构', children: list };
        })
      }else if(this.usePermit){//根据权限加载根节点 
        return this.orgFn('treeroot')(this.defaultTree).then(({result}) => {
          return {
            id: 0, 
            name: '组织架构',
    ...
    getChildren(node, pageNo){
      if(this.mode == 'user' && node.type != 'dir'){
        ...
        if(node.isdept || this.defaultRoot){//内设节点 查询下级内设（选择科室时指定了根节点只加载内设）
    ...
    listTreeNode(pid){
      return this.orgFn('listnode')(pid, this.defaultTree).then(({result}) => {
    ...
    queryTreeNode(searchkey, pagenum){
      let pid;
      if(this.defaultRoot){
        if(this.defaultRootNodeId){//查询指定节点下所有
          pid = this.defaultRootNodeId;
        }else{
          return Promise.resolve({ list: [] })
        }
      }
    ...
    queryUser(searchkey, orgid, pagenum){
      let nodeid;
      if(this.defaultRoot){
        if(this.defaultRootNodeId){//查询指定节点下所有(节点不存在时按单位查询)
          nodeid = this.defaultRootNodeId;
        }else{
    ...
    orgFn(fn){
      return this.orgApi[fn] || orgApi[fn];
    }
  }
}
</script>
