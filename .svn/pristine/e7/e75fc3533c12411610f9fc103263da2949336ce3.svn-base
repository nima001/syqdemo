...
export default {
  ...
  methods: {
    ...
    isrepeat(_id,username) {
      let result = false;
      if (this.title.indexOf("协办") >= 0) {
        for (let i in this.search) {
          if(this.search.createuser===_id) {
            ...
            result = true;
            this.clearCreateUser();
            break;
          }else if (i.indexOf("joinid") >= 0) {
            if (this.search[i] === _id&&i!==this.value) {
              this.$notification.warning({
                message: "提示",
                description: `${username} 已被选为协办人！`,
                duration: 3,
              });
              result = true;
              this.clearCreateUser();
