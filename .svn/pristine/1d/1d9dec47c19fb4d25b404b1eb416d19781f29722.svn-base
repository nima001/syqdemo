      ...
      @cancel="handleCancel">
        <a-form :form="form" :label-col="{ span: 4 }" :wrapper-col="{ span: 19 }">
          <a-form-item label="批准文号" :validateStatus="validateStatus" :help="help">
            ...
            <a-input
              type="text"
              v-decorator="[ 'zihao', { initialValue: zihao, rules: [{ required: true, message: '请输入字号!' },{validator: isNumber}], }, ]"
              style="width: 30%"
              @keyup="onChange"
            />
            <a-input
              addonBefore="〔"
              addonAfter="〕"
              v-decorator="[ 'year', { initialValue: year, rules: [{ required: true, message: '请输入年份!' },{validator: isNumber}], }, ]"
              type="text"
            ...
            <a-input
              addonAfter="号"
              type="text"
              v-decorator="[ 'ordinal', { initialValue: ordinal, rules: [{ required: true, message: '请输入序号!' },{validator: isNumber}], }, ]"
          ...
          <a-form-item label="文件归类" :wrapper-col="{ span: 10 }">
            ...
            >
              <a-select-option v-for="item in doctype" :key="item.value">{{ item.text }}</a-select-option>
            </a-select>
          </a-form-item>
          <a-form-item
            label="文件类型"
            v-if="form.getFieldValue('type') ? ![10,5].includes(form.getFieldValue('type').key) : this.subtypevisible"
            :wrapper-col="{ span: 10 }"
          >
            <a-tree-select
          ...
          <a-form-item 
            v-if="form.getFieldValue('type') ? [1, 3].includes(form.getFieldValue('type').key) : this.orgsvisible"
            label="文件归属单位"
            v-decorator="['']"
            :wrapper-col="{ span: 10 }"
            ...
            <div class="mechanism" @click="showModal(100)">
              <ul>
                ...
                <li v-for="(item, index) in orgs" :key="item.orgid">
                  {{ item.orgname }}<span @click.stop="deleteItem(index)" class="delete"><a-icon type="close" /></span>
                </li>
              </ul>
...
import { includes, difference, groupBy, values } from "lodash";
...
export default {
  props: {
    ...
    subtypevisible: {//文件类型的显隐
      type: Boolean,
    },
    orgsvisible: {//文件归属单位的显隐
      type: Boolean,
    },
    orgidselected: {//选中的单位
      type: Object,
  ...
  methods: {
    ...
    handledoctypeChange(value) {
      if (value) {
        ...
        //点击清空选择,需要将级联菜单也清空
        this.form.setFieldsValue({ subtype: undefined });
        //清空时保持节点的显示状态，节点只在切换过程中有隐藏的可能
        this.$emit('update:subtypevisible',true);
        this.$emit('update:orgsvisible',true);
