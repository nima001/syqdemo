    ...
    <a-row :gutter="20">
      <a-col :span="12">
        <a-form-item label="统计模型">
          <a-select allowClear :disabled="!!record.id" @change="form.setFieldsValue({expression: undefined})"
    ...
    <a-row :gutter="20">
      <a-col :span="12">
        <a-form-item label="字段名称">
          <a-input allowClear v-decorator="['name', { initialValue: record.name, rules: [{ required: true, message: '请输入字段名称' },]}]"/>
        </a-form-item>
      </a-col>
      <a-col :span="12">
        <a-form-item label="字段分类">
          <a-input allowClear v-decorator="['sort', { initialValue: record.sort, rules: [ { required: true, message: '请输入字段分类' },]}]"/>
        </a-form-item>
      </a-col>
    </a-row>
    <a-row :gutter="20">
      <a-col :span="12">
        <a-form-item label="输入类型" :required="true">
          <a-select allowClear v-decorator="['inputtype', { initialValue: record.inputtype, rules: [{ required: true, message: '请选择输入类型' }]}]">
      ...
      <a-col :span="12">
        <a-form-item label="字段排序">
            <a-input allowClear v-decorator="['index', { initialValue: record.index, rules: [ { pattern: /^[1-9]\d*$/, message: '请输入数字' }]}]"/>
          </a-form-item>
      </a-col>
    </a-row>
  </a-form>
