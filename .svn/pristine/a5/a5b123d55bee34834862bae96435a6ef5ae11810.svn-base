<template>
  <div class="workflow">
    <div class="userbatch" v-if="userBatchCode">
      <a-button @click="addUser">批量添加用户</a-button>
...
export default {
    ...
    this.property.childs.forEach(item => {
      ...
      if (item.componenttype == "combobox") {
        ...
        item.options.forEach(obj => {
          ...
          constantMap[obj.value] = obj.text;
        });
        ...
        this.constants[item.constant] = constantMap;
      } else if (item.componenttype == "datebox") {
        this.dateArr.push(item.code);
      }
      if(this.property.userbatchable && item.componenttype == "user"){
        if(firstuser){
          if(item.userFilter){
            this.userFilter = JSON.parse(item.userFilter);
          }
          this.userBatchCode = item.code;
          firstuser = false;
        }
      }
    });
  ...
  computed: {
    ...
    rowSelection(){
      if(this.userBatchCode){
        return {
          selectedRowKeys: this.selectedRowKeys,
          onChange: this.onSelectChange,
          getCheckboxProps: record => ({props:{disabled: (this.property.userbatchable && this.countrequired && (record.key+1) == this.data.length)}})
        }
      }else{
        return null;
      }
  ...
  methods: {
    ...
    addUser() {
      ...
      this.selectedArr = [];
      this.disabledKeysArr = this.arr.map(item => item[this.userBatchCode]._id);
    },
    deleteUser() {
      this.data = this.data.filter(item => this.selectedRowKeys.indexOf(item.key) < 0);
      this.arr = this.arr.filter(item => this.selectedRowKeys.indexOf(item.key) < 0);
      this.updataIndex();
    ...
    userOk(type, list) {
      ...
      if (type == "ok" && list.length > 0) {
        ...
        //组装用户数据然后批量添加
        list.forEach(user => {
          let dataItem = {_id: guid()};
          let arrItem = {...dataItem};
          let first = true;
          this.property.childs.forEach(c => {
            if(c.componenttype == 'user'){
              if(this.property.userbatchable){
                if(first){
                  if(!user[c.fieldsource]){
                    return;
                  }
                  dataItem[this.userBatchCode] =  user[c.fieldsource];
                  arrItem[this.userBatchCode] = {_id: user._id, [c.fieldsource]: user[c.fieldsource]};
                  first = false;
                }else{
                  if(!user[c.code]){
                    return;
                  }
                  dataItem[c.code] =  user[c.code].username;
                  arrItem[c.code] = {_id: user[c.code]._id, [c.fieldsource]: user[c.code].username};
                }
              }else{
                if(!user[c.code]){
                  return;
            ...
            }else if(c.componenttype == 'org'){
              if(this.property.orgbatchable){
                if(first){
                  if(!user[c.fieldsource]){
                    return;
                  }
                  ...
                  dataItem[c.code] =  user[c.code].name;
                  arrItem[c.code] = {_id: user[c.code]._id, [c.fieldsource]: user[c.code].name};
                }
              }else{
                if(!user[c.code]){
                  return;
                ...
                || c.componenttype == "radio"
                || c.componenttype == "combobox"){
              let constantMap = this.constants[c.constant];
              let values = (user[c.code]+"").split(",");
              if(values.length > 1){
                dataItem[c.code] =  values.map(v => constantMap[v]).join("、");
              }else if(values.length == 1){
                dataItem[c.code] =  constantMap[values[0]];
              }
              arrItem[c.code] =  user[c.code];
            }else if(c.componenttype == 'datebox'){
              let dataformat = moment(user[c.code]).format("YYYY-MM-DD");
              dataItem[c.code] =  dataformat;
              arrItem[c.code] = dataformat;
            }else if(c.componenttype == "switchbox"){
              dataItem[c.code] =  user[c.code] ? "是" : "否";
              arrItem[c.code] =  user[c.code];
            }else{
              dataItem[c.code] =  user[c.code];
              arrItem[c.code] =  user[c.code];
            }
          });
          addDatas.push(dataItem);
          addArr.push(arrItem);
        });
        this.data.push.apply(this.data, addDatas);
        this.arr.push.apply(this.arr, addArr);
        this.$store.getters.formData[this.code] = this.arr;
        this.$store.commit({
          type: "SET_FORM_DATA",
          data: this.$store.getters.formData
