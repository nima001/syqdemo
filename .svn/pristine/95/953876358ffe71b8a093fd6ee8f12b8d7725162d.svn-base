<template>
  <accordion-layout :resizeable="false">
    <div slot="nav" class="nav-panel">
      <div class="title">
        对比单位<a @click="orgSelectVisible = true"><a-icon type="plus" /></a>
      </div>
      <draggable
        class="list orgs"
        v-model="orgList"
      ...
      >
        <transition-group>
          ...
          >
            ...
            {{ item.name }}
            <a class="remove" @click="orgList.splice(index, 1)"
              ><a-icon type="delete"
            /></a>
          </li>
        </transition-group>
      ...
      <div class="title">
        对比属性<a @click="propSelectVisible = true"><a-icon type="plus" /></a>
      </div>
      <draggable
        class="list props"
        v-model="propList"
      ...
      >
        <transition-group>
          ...
          >
            <a-icon type="bars" class="sort-handle" />
            {{ item.showname }}
            <a class="remove" @click="propList.splice(index, 1)"
              ><a-icon type="delete"
            /></a>
          </li>
        </transition-group>
      ...
      <div class="footer">
        <a-button type="primary" style="width: 100%" @click="onCompare"
          >确定</a-button
        >
      </div>
      <a-modal
      ...
      >
        <org-user-select
          ...
          :selected="orgList"
          :orgApi="orgApi"
          @finish="onOrgSelected"
        />
      </a-modal>
      <a-modal
        title="选择属性"
        v-model="propSelectVisible"
        :footer="null"
        :destroyOnClose="true"
        :width="800"
        :bodyStyle="{ height: '600px', padding: '0' }"
      >
        <field-select
          :targetid="target.id"
          :multi="true"
          :default-selected="propList"
          @finish="onPropSelected"
        />
      </a-modal>
    </div>
  ...
  methods: {
    ...
    onCompare() {
      ...
      orgpropertyquery({
        ...
        orgids: this.orgList.map((item) => item._id),
        propertycodes: this.propList.map((item) => item.key),
      })
        .then((res) => {
