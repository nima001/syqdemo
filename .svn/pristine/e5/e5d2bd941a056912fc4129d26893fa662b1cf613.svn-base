<template>
  <div class="layoutcontent">
    <div class="contentbox">
      <div class="topbox">
        <div class="leftbox">
          ...
          <a-button type="primary" :style="{ marginLeft:'8px' }" @click="toTarget">统计对象管理</a-button>
        </div>
        <div class="rightbox">
          <ul>
            <li>
              <a-select :style="{ width:'160px' }" placeholder="选择统计对象" v-model="searchdata.targetid" allowClear>
                <a-select-option v-for="(item,index) in targetlist" :key="index" :value="item.id">{{item.title}}</a-select-option>
              </a-select>
            </li>
            <li>
              <a-select :style="{ width:'160px' }" placeholder="选择字段分类" allowClear v-model="searchdata.sort">
                <a-select-option v-for="(item,index) in fieldList" :key="index" :value="item">{{item}}</a-select-option>
              </a-select>
            </li>
            <li>
              <a-input allow-clear placeholder="输入字段名称或编码" allowClear :style="{ width:'200px' }" v-model="searchdata.searchKey"/>
            </li>
            <li>
      ...
      <div class="tablecount">
        <a-table rowKey="id" :row-selection="{selectedRowKeys:selectedRowKeys,onChange:onSelectChange}"  :loading="loading" :columns="columns" :customRow="customRow" :dataSource="pagination.rows" :pagination="false">
    ...
    <statistics-change v-model="visible" :record="record" :targetlist="targetlist" :modelList="modelList" @finish="onEditFinish"/>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    toRules(field){
      if(field.code){
        this.$router.push({name: "RulesList",params:{field}});
      }
    },
    onreset() {
