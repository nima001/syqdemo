...
export default {
  props: {},
  components: { SelectCondition, TrendChart, PercentChart, AttributeChart },
  data() {
    return {
      loading: false,
      title: '注册',
  ...
  methods: {
    getData(data) {
      ...
      .then(({attr}) => {
        ...
        //  构造趋势图数据
        this.trandData = this.handleData( attr.time, attr.regNumList);
        this.percentData = [attr.regAppidList, attr.regOrgIdList, attr.regsourceList];
        //  排序 获取前十
        ...
        this.attributeData.topList = topList.slice(0, 10);
