<template>
  <div class="orgmanager-detail" ref="orgmanagerDetail">
    <div class="orgmanagerlist">
      <div class="top">
        <div class="left">
      ...
      <a-table
       class="per-table"
        ...
        :customRow="customRow"
        :rowClassName="(row) => pagination.selected && pagination.selected.id == row.id ? 'selected': ''"
        :pagination="false"
        :loading="loading"
...
import {querypagelist} from "@/idm/api/orgmanager";
...
export default {
  ...
  data(){
    return{
      columns: [
        { title: "姓名", width: "110px", dataIndex: "name", scopedSlots: { customRender: "name" } },
        { title: "账号", width: "110px", dataIndex: "loginname", scopedSlots: { customRender: "loginname" } },
        { title: "手机号码", width: "110px", dataIndex: "mobilephone", scopedSlots: { customRender: "mobilephone" } },
        { title: "所在组织", width: "110px", dataIndex: "orgname", scopedSlots: { customRender: "orgname" } },
      ],
      choice: 1,
      properties: {},
      keyWord: undefined,
  ...
  created() {
    this.getOrgManager(this.pagination);
  },
  watch: {
    org(val){
      this.getOrgManager(this.pagination);
  ...
  methods:{
    ...
    removeitem(){
      for(var i in this.selectedRowKeys){
        this.pagination.rows = this.pagination.rows.filter(item => item.id !== this.selectedRowKeys[i])
      }
    },
    getParams(page) {
      const params = {
          orgid: this.org._id,
          // orgid: '006099c6672944469bdfc832e3ccbbba',
          needtotal: true,
          ...page
          // pagesize: 2,
          // pagenum: 1,
