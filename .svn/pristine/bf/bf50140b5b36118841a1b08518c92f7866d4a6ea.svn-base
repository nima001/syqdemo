<template>
  <div class="content">
    <div class="head">
      <span>平均分</span>
      <span class="num">
        <span v-for="(item, index) in aveNum"  :key="index"
    ...
    <div class="body">
      <table>
        ...
        <tbody>
          <tr v-for="item in orgList" :key="item.id" @click="choose(item)" :class="{'active': isActive(item.id)}">
...
export default {
  ...
  computed: {
    ...
    orgList(){
      if(this.data){
        return sortBy(this.data, 'current')
      }
    }
  },
  methods: {
