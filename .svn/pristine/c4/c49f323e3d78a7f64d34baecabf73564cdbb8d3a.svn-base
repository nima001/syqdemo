...
export default {
  ...
  computed: {
    ...
    totalField(){
      return `_id@organization.statistic.${this.type == "school" ? "xxgyzszf": "yygyzszf"}`;
    }
  },
  // 公益指数过滤
  filters:{
  ...
  methods: {
    openOrgList(){
      this.orgList = {
        ...
        loader: (page) => {
          return this.request([
            { key: this.totalField, showname: '总分' }
          ], page).then(({result: {total, pagenum, pagesize, header, rows}}) => {
            let col = header.find(item => item.key == this.totalField);
            return {
              result: {
                total, pagenum, pagesize,
                rows: rows.map(item => {
                  let num = item[col.column];
                  return {
                    _id: item._id,
                    name: item.name,
                    num: num && num.toFixed(1)
                  }
                })
              }
            }
    ...
    removeSuffix(header, list) {
      let array = [];
      let totalItem = header.find((item) => item.key == this.totalField);
    ...
    request(fields, page) {
      ...
      return query({
        ...
        fields: [
          ...
          { key: this.totalField, name: "总分" }, 
          ...fields
        ],
        orders: [
          { orderby: this.totalField, ordertype: "DESC" },
